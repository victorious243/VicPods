...
function normalizeSuggestion(item) {
  ...
  const type = clampText(item?.type, 32) || 'Suggestion';
  const detail = clampText(item?.detail, 220);
...
function buildDefaultSuggestions() {
  return [
    {
      type: 'Episode idea',
      title: 'The workflow bottleneck that keeps weekly podcasts inconsistent',
      detail: 'Break down the one process gap that slows your recording rhythm and turn it into a practical episode.',
    },
    {
      type: 'Improvement',
      title: 'Tighten your hook before you write the rest of the draft',
      detail: 'Start from a sharper opening angle so the full episode carries more tension and clarity.',
    },
    {
      type: 'Episode idea',
      title: 'A behind-the-scenes episode on how you plan from idea to publish',
      detail: 'Use your own workflow as the story and turn it into a repeatable framework listeners can borrow.',
    },
  ];
}

function buildAiSuggestionsEmail({
  name,
  appUrl,
  suggestions = [],
  introNote,
  ...
  const createUrl = `${normalizedAppUrl}/create/single`;
  const normalizedSuggestions = (Array.isArray(suggestions) ? suggestions : [])
    .map(normalizeSuggestion)
    .filter(Boolean);
  const finalSuggestions = normalizedSuggestions.length > 0 ? normalizedSuggestions : buildDefaultSuggestions();
  const note = String(introNote || '').trim()
    || 'Here are a few smart starting points to help you shape the next episode or strengthen the draft you already have.';
  const subject = 'Fresh AI suggestions for your next VicPods episode';
  const previewText = 'New episode ideas and improvement angles are ready to explore in VicPods.';
  ...
  const text = [
    ...
    note,
    '',
    ...finalSuggestions.flatMap((suggestion, index) => [
      `${index + 1}. ${suggestion.type}: ${suggestion.title}`,
      suggestion.detail ? `   ${suggestion.detail}` : '',
      '',
    ]),
    `Explore ideas: ${pantryUrl}`,
    `Start an episode: ${createUrl}`,
    '',
    'VicPods Team',
            ...
            <tr>
              <td style="padding:32px 32px 24px; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif; color:#0f172a;">
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin:0 auto 18px;">
                  <tr>
                    <td align="center" bgcolor="#5b5ff8" style="border-radius:14px;">
                      <a href="${escapeHtml(pantryUrl)}" style="display:inline-block; padding:15px 24px; font-size:15px; line-height:1; font-weight:700; color:#ffffff; text-decoration:none; border-radius:14px;">
                        Explore ideas
                      </a>
                    </td>
                  </tr>
                ...
                <p style="margin:0; font-size:14px; line-height:1.7; color:#475569; text-align:center;">
                  Want to turn one into a full draft now?
                  <a href="${escapeHtml(createUrl)}" style="color:#5b5ff8; text-decoration:none; font-weight:600;">Start an episode</a>
                </p>
              </td>
            </tr>
            <tr>
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-top:1px solid #e2e8f0;">
                  <tr>
                    <td style="padding-top:20px; font-size:13px; line-height:1.7; color:#64748b; text-align:center;">
                      VicPods helps creators turn rough ideas into structured episodes with less friction.
                    </td>
                  </tr>
                  <tr>
...
async function sendAiSuggestionsEmail(input) {
  const logoAttachment = buildEmailLogoAttachment();
  const email = buildAiSuggestionsEmail({
    ...input,
...
module.exports = {
  buildAiSuggestionsEmail,
  sendAiSuggestionsEmail,
};
